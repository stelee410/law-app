...
export function CaseCard({ lawCase }: { lawCase: LawCase }) {
  ...
  return (
    <Link
      ...
      className="block rounded-lg border border-slate-100 bg-white p-4 shadow-sm"
    >
      <div className="flex items-start gap-3">
        ...
        <span className="min-w-0 flex-1">
          <span className="flex min-w-0 items-start justify-between gap-2">
            <strong className="min-w-0 flex-1 break-words text-base leading-5 text-slate-950">{lawCase.debtorName}</strong>
            <em className="shrink-0 rounded-full bg-blue-50 px-2 py-1 text-[11px] font-bold not-italic text-blue-700">
              {lawCase.status}
            </em>
          </span>
